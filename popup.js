...
async function analyzePoliticalBias(articleContent) {
  ...
  // Try different API endpoints in case of compatibility issues
  const endpoints = [
    { url: `${apiUrl}/api/chat`, body: { model: 'llama2', messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }], stream: false }},
    { url: `${apiUrl}/api/generate`, body: { model: 'llama2', prompt: `${systemPrompt}\n\nUser: ${userPrompt}\nAssistant:`, stream: false }},
    { url: `${apiUrl}/v1/chat/completions`, body: { model: 'llama2', messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }], stream: false }}
  ];
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.message && data.message.content) {
          analysis = data.message.content; // Ollama format
        } else if (data.response) {
          analysis = data.response; // llama.cpp format
        } else if (data.choices && data.choices[0] && data.choices[0].message) {
          analysis = data.choices[0].message.content; // OpenAI-compatible format
        } else {
  ...
  if (lastError.message.includes('403')) {
    errorMessage += `
      <div style="margin-top: 10px; padding: 10px; background: #fff3cd; border: 1px solid #ffeaa7; border-radius: 3px; font-size: 11px;">
        ...
        • Ensure Ollama is running: <code>ollama serve</code><br>
        • Check server accessibility: <code>curl ${apiUrl}/api/chat</code><br>
        • Try restarting Ollama: <code>ollama stop && ollama serve</code><br>
        • Verify model is loaded: <code>ollama list</code><br>
        • Try different port: <code>ollama serve --port 11435</code>
      </div>
    `;
  } else {
    errorMessage += `<small>Make sure your LLAMA server is running at ${apiUrl}</small>`;
